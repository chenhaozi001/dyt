...
> 🌟 **不懂代码？两种用法 →**
> - 想要**一个网址、点链接就能用**（可分享给同事）：看「[免费部署成网站](#-免费部署成网站点链接就能用)」。
> - 想在**自己电脑本地**用：看「[网页版通话复盘助手](#-网页版通话复盘助手不用懂代码)」，双击启动即可。
> 两种都**不用敲命令**。
...
想要一个**网址**、在任何浏览器打开就能用、还能发给同事？把 `app.py` 免费部署到
Streamlit Community Cloud 即可（**全程免费、不用信用卡、不用写代码**）。

- AI 接口：用**免费的 Groq Key**（不用绑卡），或公司自己的接口。
- 详细的保姆级图文步骤见 **`部署指南.md`**。

> 注意：Cursor 会员只能在 Cursor 里用，不能给这个外部网站调用 AI，所以需要单独领一个免费 AI Key（指南里教）。
...
├── app.py                         在线版网站（免费部署到 Streamlit Cloud）
├── 部署指南.md                    保姆级免费部署步骤
├── .streamlit/config.toml         在线版网站配色
├── webapp/                        本地版网页助手（双击启动，不用懂代码）
